...
function Node(image, x, y, width, height) {
	...
	this.isInDrawingLinePointBound = function(x, y) {
		if (this.isInBound(x, y)) {
			...
			// left or right
			if (this.y + (this.height) / 2 - radius >= y && this.y + (this.height) / 2 + radius <= y) {
				if (this.x >= x && this.x + 2 * radius <= x) {
					return DrawingLinePointDirection.LEFT;
				} else if (this.x + this.width - 2 * radius >= x && this.x + this.width <= x) {
					return DrawingLinePointDirection.RIGHT;
				}
			}
			// up or down
			else if (this.x + (this.width) / 2 - radius >= x && this.x + (this.width) / 2 + radius <= x) {
				// up
				if (this.y >= y && this.y + 2 * radius <= y) {
					return DrawingLinePointDirection.UP;
				}
				// down
				else if (this.y + this.height - 2 * radius >= y && this.y + this.height <= y) {
